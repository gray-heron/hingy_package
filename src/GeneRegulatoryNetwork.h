...
template <int D, typename scalartype = float> class Protein
{
  private:
...
class GeneRegulatoryNetwork : public Trainable<scalartype>
{
  ...
  public:
    ...
    virtual void Mutate(Randomizer<scalartype> &generator)
    {
        ...
        if (chromosome)
        {
            do
            {
                ...
                if (mutation_type == 0 && proteins.size() < 30)
                { // create protein
                    proteins.insert(proteins.begin() + input_proteins_n,
                                    Protein<D, scalartype>(
                                        (float)protein_density, generator));
                ...
                { // mutate_protein
                    uint32_t protein_index =
                        generator.RandomInt(0, proteins.size() - 1);
    ...
    virtual std::vector<std::shared_ptr<Trainable<scalartype>>>
    Crossover(Trainable<scalartype> &other, Randomizer<scalartype> &generator)
    {
        ...
        try
        {
            ...
            uint32_t crossover_marker1 = generator.RandomInt(
                input_proteins_n, proteins.size() - output_proteins_n);
            uint32_t crossover_marker2 = generator.RandomInt(
                input_proteins_n,
                other_grn.proteins.size() - output_proteins_n);

            typename std::vector<Protein<D, scalartype>>::iterator
                crossover_point1 = proteins.begin() + crossover_marker1;
            typename std::vector<Protein<D, scalartype>>::iterator
                crossover_point2 =
            ...
            auto child1 =
                std::make_shared<GeneRegulatoryNetwork<D, scalartype>>(
                    input_proteins_n, output_proteins_n, protein_density,
                    generator, false);
            auto child2 =
                std::make_shared<GeneRegulatoryNetwork<D, scalartype>>(
                    input_proteins_n, output_proteins_n, protein_density,
            ...
            child1->proteins.insert(child1->proteins.end(), proteins.begin(),
                                    crossover_point1);
            child1->proteins.insert(child1->proteins.end(), crossover_point2,
                                    other_grn.proteins.end());

            child2->proteins.insert(child2->proteins.end(),
                                    other_grn.proteins.begin(),
                                    crossover_point2);
            child2->proteins.insert(child2->proteins.end(), crossover_point1,
                                    proteins.end());

            if (generator.RandomInt(0, 2))
                child1->beta = this->beta;
            else
                child1->beta = other_grn.beta;
            if (generator.RandomInt(0, 2))
                child2->beta = this->beta;
            else
                child2->beta = other_grn.beta;
            if (generator.RandomInt(0, 2))
                child1->delta = this->beta;
            else
                child1->delta = other_grn.delta;
            if (generator.RandomInt(0, 2))
                child2->delta = this->beta;
            else
                child2->delta = other_grn.delta;

            child1->concentrations.resize(child1->proteins.size());
            child2->concentrations.resize(child2->proteins.size());

            ret.push_back(
                std::dynamic_pointer_cast<Trainable<scalartype>>(child1));
            ret.push_back(
                std::dynamic_pointer_cast<Trainable<scalartype>>(child2));
        }
        catch (std::bad_cast exception)
        {
            fprintf(stderr,
                    "You can not crossover a GRN with a non-GRN trainable!\n");
        }
    ...
    virtual std::vector<scalartype> Step(std::vector<scalartype> inputs)
    {
        ...
        {
            fprintf(stderr,
                    "Wrong number of input protein concenrations given!\n");
        }
        ...
        for (auto protein = proteins.begin() + input_proteins_n;
             protein != proteins.end(); ++protein)
        {
            scalartype change = 0;
            uint32_t protein_id = protein - proteins.begin();

            // don't accect output proteins as efectors
            for (auto efector = proteins.begin();
                 efector != proteins.end() - output_proteins_n; ++efector)
            {
        ...
        if (theta != 0.0f)
            for (auto concentration =
                     new_concentrations.begin() + input_proteins_n;
        ...
        return std::vector<scalartype>(
            concentrations.begin() + (proteins.size() - output_proteins_n),
            concentrations.end());
    }

    virtual void Configure(uint32_t input_proteins_n,
                           uint32_t output_proteins_n,
    ...
    virtual int GetMaxSerializedBufferSize() const
    {
        return proteins.size() * sizeof(Protein<D, scalartype>) +
               ...
               5 * sizeof(uint32_t);    // density, inputs_n, outputs_n,
                                     // regulatory_n, D
    }
    ...
    virtual std::vector<uint8_t> Serialize() const
    {
        ...
        uint32_t regulatory_proteins_n =
            proteins.size() - input_proteins_n - output_proteins_n;

        memcpy(cursor, (void *)&input_proteins_n, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy(cursor, (void *)&output_proteins_n, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy(cursor, (void *)&regulatory_proteins_n, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy(cursor, (void *)&protein_density, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy(cursor, (void *)&d, sizeof(uint32_t));
        cursor += sizeof(uint32_t);

        memcpy(cursor, (void *)&delta, sizeof(uint32_t));
        cursor += sizeof(scalartype);
        memcpy(cursor, (void *)&beta, sizeof(uint32_t));
        cursor += sizeof(scalartype);

        memcpy(cursor, proteins.data(),
               proteins.size() * sizeof(Protein<D, scalartype>));
    ...
    virtual void Deserialize(const std::vector<uint8_t> &data_vector)
    {
        ...
        memcpy((void *)&input_proteins_n, cursor, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy((void *)&output_proteins_n, cursor, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy((void *)&regulatory_proteins_n, cursor, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy((void *)&protein_density, cursor, sizeof(uint32_t));
        cursor += sizeof(uint32_t);
        memcpy((void *)&d, cursor, sizeof(uint32_t));
        cursor += sizeof(uint32_t);

        memcpy((void *)&delta, cursor, sizeof(uint32_t));
        cursor += sizeof(scalartype);
        memcpy((void *)&beta, cursor, sizeof(uint32_t));
        cursor += sizeof(scalartype);

        assert(d == D);

        concentrations.resize(input_proteins_n + output_proteins_n +
                              regulatory_proteins_n);
        for (int i = 0; i < concentrations.size(); i++)
        {
            proteins.push_back(
                Protein<D, scalartype>((const char *)cursor, protein_density));
