...
bool parse_arguments(string name_prefix, char name_value_sep, int argc,
                     char **argv, stringmap &out)
{
    ...
    for (int arg_i = 0; arg_i < argc; arg_i++)
    {
        ...
        {
            log_warning((string) "Wrong prefix on argument: " +
                        current_argument + "!");
            clean = false;
            continue;
        }
        ...
        {
            log_warning((string) "No separator on argument: " +
                        current_argument + "!");
            clean = false;
            continue;
        }
                          ...
                          stringmap &out)
{
    if (file_exists(filename))
    {
             ...
             param_node = param_node->next_sibling())
        {
            ...
            {
                log_info((string) "Ignoring " + param_name +
                         " from the xml parameters!");
                continue;
            }
