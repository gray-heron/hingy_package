...
public:
    ...
    virtual vector<shared_ptr<Trainable<scalartype>>> Crossover(Trainable<scalartype>& other, Randomizer<scalartype>& generator) = 0;
...
class Trainer {
    static_assert(std::is_base_of<Trainable<scalartype>, TTrainable>::value, "You can only train Trainables :)");

    typedef typename vector<pair<shared_ptr<Trainable<scalartype>>, scalartype>>::iterator marker_t;
    ...
    static int CompareFitness(
        pair<shared_ptr<Trainable<scalartype>>, scalartype> a,
...
public:
    ...
    scalartype Generation(Randomizer<scalartype>& generator) {
        ...
        auto worker_function =
            [this, &newborns, &generator]
        (int thread_id, population_range_t range) mutable -> void
        {
            for (auto i = range.first; i != range.second; i++) {
                if (crossover_rate >= generator.RandomScalar(0.0, 1.0)) {
                    auto  partner_id = generator.RandomInt(0, this->population.size());
                    auto& partner = this->population[partner_id].first;

                    for (const auto& child : i->first->Crossover(*partner, generator)) {
                        child->Reset();
                        newborns[thread_id].push_back(std::make_pair<shared_ptr<Trainable<scalartype>>, scalartype>(
                ...
                if (mutation_rate >= generator.RandomScalar(0.0, 1.0)) {
                    std::shared_ptr<Trainable<scalartype>> new_child =
                        std::make_shared<TTrainable>(dynamic_cast<TTrainable&>(*i->first));
            ...
            sort(newborns[thread_id].begin(), newborns[thread_id].end(), CompareFitness);
        };
        ...
        for (int thread_id = 0; thread_id < THREADS_N; thread_id++) {
            threads.push_back(std::thread(
                ...
                std::make_pair<marker_t, marker_t>(
                    ...
                    population.begin() + ((thread_id + 1) * population_per_thread)
                    )
            ));
        }
    ...
    void Purge(scalartype alphas_f, Randomizer<scalartype>& generator) {
        ...
        uint32_t newborns_per_thread = int(float(to_wipe_out_n) / float(THREADS_N));
        ...
        for (int thread_id = 0; thread_id < THREADS_N - 1; thread_id++) {
            threads.push_back(std::thread(
                worker_function,
                std::make_pair<marker_t, marker_t>(
                    population.begin() + alphas_n + ((thread_id)* newborns_per_thread),
                    population.begin() + alphas_n + ((thread_id + 1) * newborns_per_thread)
                    )
            ));
        }
        ...
        worker_function(
            std::make_pair<marker_t, marker_t>(
                population.begin() + alphas_n + (THREADS_N - 1) * newborns_per_thread,
                population.end()
                )
        );
    ...
    Trainer(uint32_t population_size, uint32_t inputs_n, uint32_t outputs_n, float mutation_rate, float crossover_rate,
        function<scalartype(shared_ptr<Trainable<scalartype>>)> fitness_function, vector<scalartype>& params,
        Randomizer<scalartype>& generator)
        :
        inputs_n(inputs_n), outputs_n(outputs_n),
        crossover_rate(crossover_rate), mutation_rate(mutation_rate), population_size(population_size),
        fitness_function(fitness_function), params(params)
    {
        ...
        for (unsigned int i = 0; i < population_size; i++) {
            ...
            (population.end() - 1)->first->Configure(inputs_n, outputs_n, params, generator);

            for (int i = 0; i < INITIAL_MUTATIONS_N; i++)
                (population.end() - 1)->first->Mutate(generator);

            (population.end() - 1)->second = fitness_function(
               (population.end() - 1)->first
            );
        }

        std::sort(population.begin(), population.end(), CompareFitness);

        population_per_thread = int(float(population.size()) / float(THREADS_N) * PPP);
        alphas_n = int(float(population.size()) * PPP);
    }

    Trainer(char* buf, function<scalartype(shared_ptr<Trainable<scalartype>>)> fitness_function) :
        fitness_function(fitness_function)
    {
        uint32_t cursor = 0;

        memcpy((void*)&population_size, &buf[cursor], sizeof(uint32_t)); cursor += sizeof(uint32_t);
        memcpy((void*)&inputs_n, &buf[cursor], sizeof(uint32_t)); cursor += sizeof(uint32_t);
        memcpy((void*)&outputs_n, &buf[cursor], sizeof(uint32_t)); cursor += sizeof(uint32_t);

        memcpy((void*)&mutation_rate, &buf[cursor], sizeof(scalartype)); cursor += sizeof(scalartype);
        memcpy((void*)&crossover_rate, &buf[cursor], sizeof(scalartype)); cursor += sizeof(scalartype);
        ...
        for (int i = 0; i < population_size; i++) {
            ...
            memcpy((void*)&(population.end() - 1)->second, &buf[cursor], sizeof(scalartype));
    ...
    virtual uint32_t Serialize(char* buf) const {
        ...
        memcpy(&buf[cursor], (void*)&population_size, sizeof(uint32_t)); cursor += sizeof(uint32_t);
        memcpy(&buf[cursor], (void*)&inputs_n, sizeof(uint32_t)); cursor += sizeof(uint32_t);
        memcpy(&buf[cursor], (void*)&outputs_n, sizeof(uint32_t)); cursor += sizeof(uint32_t);

        memcpy(&buf[cursor], (void*)&mutation_rate, sizeof(scalartype)); cursor += sizeof(scalartype);
        memcpy(&buf[cursor], (void*)&crossover_rate, sizeof(scalartype)); cursor += sizeof(scalartype);

        for (const auto& i : population) {
            cursor += i.first->Serialize(&buf[cursor]);
            memcpy(&buf[cursor], (void*)&i.second, sizeof(scalartype)); cursor += sizeof(scalartype);
        }
